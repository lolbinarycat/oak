...
func TestRectConstructors(t *testing.T) {
	...
	assert.Equal(t, Rect3{Min: Point3{0, 0, 0}, Max: Point3{1, 1, 1}},
		NewBoundingRect3(Point3{0, 0, 0}, Point3{0, 1, 1}, Point3{1, 1, 0}, Point3{1, 0, 1}))
}

func TestRectAccess(t *testing.T) {
	r2 := NewRect2(0, 1, 2, 3)
	r3 := NewRect3(0, 1, 2, 3, 4, 5)
	assert.Equal(t, 4, r2.Area())
	assert.Equal(t, 27, r3.Space())
	assert.Equal(t, 2, r2.W())
	assert.Equal(t, 2, r2.H())
	assert.Equal(t, 3, r3.W())
	assert.Equal(t, 3, r3.H())
	assert.Equal(t, 3, r3.D())
	assert.Equal(t, 1, r2.Midpoint(0))
	assert.Equal(t, 2, r2.Midpoint(1))
	assert.Equal(t, 1, r3.Midpoint(0))
	assert.Equal(t, 2, r3.Midpoint(1))
	assert.Equal(t, 3, r3.Midpoint(2))
	assert.Equal(t, 8, r2.Perimeter())
	assert.Equal(t, 36, r3.Margin())
}
